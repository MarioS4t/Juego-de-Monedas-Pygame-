...
class Game:
    ...
    def generate_coins(self):
        ...
        for c in range(0, MAX_COINS):
            pos_x = random.randrange(last_position + 100, last_position + 300)
    ...
    def draw_text(self):
        ...
        if not self.playing:
            self.display_text("Game Over", 36, RED, WIDTH // 2, HEIGHT // 2)
            self.display_text("Press r to play again", 36, BLACK, WIDTH // 2, HEIGHT // 2 + 50)
    ...
    def display_text(self,tex, size, color, pos_x, pos_y):
        font = pygame.font.Font(self.font, size)

        text = font.render(tex, True, color)
        rect = text.get_rect()
    ...
    def menu(self):
        ...
        self.display_text("Press any key to start", 36, WHITE, WIDTH // 2, HEIGHT // 2)
        pygame.display.flip()
        self.wait()
